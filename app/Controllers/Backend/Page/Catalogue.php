...
class Catalogue extends BaseController{
	...
	public function index($page = 1){
		...
		$flag = $this->authentication->check_permission([
			'routes' => 'backend/page/catalogue/index'
		]);
		if($flag == false){
		...
		$config['total_rows'] = $this->AutoloadModel->_get_where([
			...
			'join' =>  [
				[
					'page_translate as tb2','tb1.id = tb2.objectid AND tb2.module = \''.$this->data['module'].'\'   AND tb2.language = \''.$this->currentLanguage().'\' ','inner'
				],
			],
			'keyword' => $keyword,
			'where' => $where,
		...
		if($config['total_rows'] > 0){
			$config = pagination_config_bt(['url' => 'backend/page/catalogue/index','perpage' => $perpage], $config);
			...
			$this->data['pageCatalogueList'] = $this->AutoloadModel->_get_where([
		...
		$this->data['template'] = 'backend/page/catalogue/index';
	...
	public function create(){
		$session = session();
		$flag = $this->authentication->check_permission([
			'routes' => 'backend/page/catalogue/create'
		]);
		if($flag == false){
 			$this->session->setFlashdata('message-danger', 'Bạn không có quyền truy cập vào chức năng này!');
 			return redirect()->to(BASE_URL.'backend/dashboard/dashboard/index');
			...
			if ($this->validate($validate['validate'], $validate['errorValidate'])){
		 		...
		 		if($resultid > 0){
					...
					$this->nestedsetbie->Action();
 					...
 					return redirect()->to(BASE_URL.'backend/page/catalogue/create');
		 		}
		...
		$this->data['template'] = 'backend/page/catalogue/create';
		return view('backend/dashboard/layout/home', $this->data);
	}
	...
	public function update($id = 0){
		...
		$session = session();
		$flag = $this->authentication->check_permission([
			'routes' => 'backend/page/catalogue/update'
		]);
		if($flag == false){
 			$this->session->setFlashdata('message-danger', 'Bạn không có quyền truy cập vào chức năng này!');
 			return redirect()->to(BASE_URL.'backend/dashboard/dashboard/index');
		...
		$this->data[$this->data['module']] = $this->AutoloadModel->_get_where([
			'select' => 'tb1.id, tb2.title, tb2.canonical, tb2.description, tb2.content, tb2.meta_title, tb2.meta_description, tb1.parentid, tb1.image, tb1.album, tb1.publish, tb2.router, tb2.icon, tb2.template',
			'table' => $this->data['module'].' as tb1',
			'join' =>  [
					[
						'page_translate as tb2','tb1.id = tb2.objectid AND tb2.module = \''.$this->data['module'].'\'   AND tb2.language = \''.$this->currentLanguage().'\'  ','inner'
		...
		if(!isset($this->data[$this->data['module']]) || is_array($this->data[$this->data['module']]) == false || count($this->data[$this->data['module']]) == 0){
			$session->setFlashdata('message-danger', 'Nhóm Bài Viết không tồn tại');
 			return redirect()->to(BASE_URL.'backend/page/catalogue/index');
		}
		...
		if($this->request->getMethod() == 'post'){
			...
			if ($this->validate($validate['validate'], $validate['errorValidate'])){
		 		...
		 		if($flag > 0){
		 			$flag = $this->AutoloadModel->_update([
			 			'table' => 'page_translate',
			 			'where' => ['objectid' => $id,'module' => 'page_catalogue', 'language' => $this->currentLanguage()],
			 			'data' => $updateLanguage
			 		]);
		...
		$this->data['template'] = 'backend/page/catalogue/update';
		return view('backend/dashboard/layout/home', $this->data);
	}
	...
	public function delete($id = 0){
		$session = session();
		$flag = $this->authentication->check_permission([
			'routes' => 'backend/page/catalogue/delete'
		]);
		if($flag == false){
 			$this->session->setFlashdata('message-danger', 'Bạn không có quyền truy cập vào chức năng này!');
 			return redirect()->to(BASE_URL.'backend/dashboard/dashboard/index');
		...
		$this->data['template'] = 'backend/page/catalogue/delete';
		return view('backend/dashboard/layout/home', $this->data);
	}
